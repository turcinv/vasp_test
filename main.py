...
def clean_xyz_file(input_file: str, output_file: str):
    """Removes non-numeric headers from XYZ files before loading them with mdtraj."""
    with open(input_file, "r") as infile, open(output_file, "w") as outfile:
        for line in infile:
            if "frame" in line or "Written by MDAnalysis" in line:
                continue  # Skip MDAnalysis headers
            outfile.write(line)  # Write only valid XYZ content
